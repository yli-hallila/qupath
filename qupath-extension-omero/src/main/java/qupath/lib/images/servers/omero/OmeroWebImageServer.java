...
//				.args(args)
		...
		for (PathObject pathObject : getROIs()) {
			PathObjects.createAnnotationObject(pathObject.getROI(), PathPrefs.getAutoSetAnnotationClass());
		}
	}
			...
			for (int i = 0; i < roisJson.size(); i++) {
				...
				for (int j = 0; j < shapesJson.size(); j++) {
					...
					if (type.toLowerCase().endsWith("#line")) {
						...
						roi = ROIs.createLineROI(x, y, x2, y2, plane);
