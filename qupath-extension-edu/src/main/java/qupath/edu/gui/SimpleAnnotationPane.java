...
//			Platform.runLater(() -> selectedPathObjectChanged(pathObjectSelected, previousObject, allSelected));
        ...
        if (pathObjectSelected instanceof PathAnnotationObject) {
            ...
            if (annotation.getDescription() != null && annotation.retrieveMetadataValue(ANSWER_KEY) == null) {
                descriptionProperty.set(annotation.getDescription());
                answerProperty.set(null);
            } else {
                descriptionProperty.set(null);
                answerProperty.set((String) annotation.retrieveMetadataValue(ANSWER_KEY));
            }
        } else {
            answerProperty.set(null);
        }
    ...
    private void showQuizDialog(PathObject pathObject) {
        try {
            List<MultichoiceOption> choices = List.of(new Gson().fromJson((String) pathObject.retrieveMetadataValue(ANSWER_KEY), MultichoiceOption[].class));
    ...
    private void showAnswerDialog(PathObject pathObject) {
        Dialogs.showMessageDialog(pathObject.getName(), (String) pathObject.retrieveMetadataValue(ANSWER_KEY));
    }
